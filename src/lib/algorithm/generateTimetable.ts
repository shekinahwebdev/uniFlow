...
export function generateTimetable(
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    let scheduled: TimetableEntry;
    try {
      scheduled = assignVenue(scheduledEntry, [...globalEntries, ...timetable]);
    } catch {
      continue;
    }

    const allEntries = [...globalEntries, ...timetable];
    const hasConflict = checkConflicts(scheduled, allEntries);
    if (hasConflict) continue;

    timetable.push(scheduled);
  }

  const isValid = validateSchedule([...globalEntries, ...timetable]);

  if (!isValid) {
    throw new Error("Generated timetable contains conflicts.");
  }
